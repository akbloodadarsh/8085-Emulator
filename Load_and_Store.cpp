...
void LDA(string &user_instruction)
{
	string address = "";
	for (int i = 4; i <= 7; ++i)
		address.push_back(user_instruction[i]);
...
void LXI(string &user_instruction)
{
	...
	for (int i = 7; i <= 8; ++i)
		address1.push_back(user_instruction[i]);
	for (int i = 9; i <= 10; ++i)
		address2.push_back(user_instruction[i]);
	switch (user_instruction[4])
	{
	case 'B':
