...
void INX(string &user_instruction)
{
	...
	++temp;
	if (temp > hex_to_dec("FFFF"))
		temp -= hex_to_dec("FFFF");
	string val = dec_to_hex16(temp);
	string address1 = "", address2 = "";
	for (int i = 0; i <= 1; ++i)
		address1.push_back(val[i]);
	for (int i = 2; i <= 3; ++i)
		address2.push_back(val[i]);
	switch (user_instruction[4])
	{
	case 'B':
