...
int main()
{
	...
	if (debugger_mode)
	{
		while (check)
		{
			...
			for (int i = 0; i < user_instruction.size(); ++i)
				user_instruction[i] = tolower(user_instruction[i]);
			int i = 0;
			...
			if (mnemonics.find(mne) == mnemonics.end())
				cout << "Debugger Instruction not found\nRe-enter correct Debugger Instruction or type HELP\n";
			else if (mnemonics[mne].first == 35 || mnemonics[mne].first == 36)
				cout << "This instruction can't be performed at this moment\n";
			else if (mnemonics[mne].first == 28)
				help_debug();
			...
			else if (mnemonics[mne].first == 37 || mnemonics[mne].first == 38)
				check = 0;
		}
	}
	...
	if (file_mode)
	{
		...
		while (getline(fp, user_instruction))
		{
			mne = "";
			for (int i = 0; i < user_instruction.size(); ++i)
				user_instruction[i] = toupper(user_instruction[i]);
			i = 0;
			while (i < user_instruction.size() && user_instruction[i] != ' ')
			{
				mne.push_back(user_instruction[i]);
			...
			else if (instruction_size[mnemonics[mne].first].first != user_instruction.size())
			{
				...
				{
					cout << "Did you mean:-  ";
					for (int i = 0; i < instruction_size[mnemonics[mne].first].first - 1; ++i)
						cout << user_instruction[i];
	...
	if (debugger_mode && break_line != -1)
		end_address = break_line;
	next_address = hex_to_dec(starting_address);
	while (next_address < end_address)
	{
		if (address_instruction.find(next_address) != address_instruction.end())
		{
			...
			{
				string temp;
				for (int i = 0; i < 4 - temp_address.size(); ++i)
					temp.push_back('0');
				temp += temp_address;
				temp_address = temp;
			}
			if (!no_extra_print)
			{
				...
				for (int i = display_content[next_address].size(); i <= 25; ++i)
					display_content[next_address].push_back(' ');
			}
			no_extra_print = 0;
			for (int i = 0; i < user_instruction.size(); ++i)
				user_instruction[i] = toupper(user_instruction[i]);
				...
				if (temp < next_address)
				{
					for (int j = temp + 1; j < next_address; ++j)
					{
						if (address_instruction.find(j) != address_instruction.end())
						{
							...
							{
								string tem;
								for (int i = 0; i < 4 - temp_addr.size(); ++i)
									tem.push_back('0');
								tem += temp_addr;
								temp_addr = tem;
							}
							...
							for (int i = display_content[j].size(); i <= 25; ++i)
								display_content[j].push_back(' ');
						...
						if (run != 1)
						{
							while (check)
							{
								...
								else if (mnemonics[mne].first == 35 || mnemonics[mne].first == 36)
								{
									...
									{
										if (address_data.find(hex_to_dec(reg_or_add)) != address_data.end())
											cout << address_data[hex_to_dec(reg_or_add)] << "\n";
										else
											cout << "No data present at the given address " << reg_or_add << "\n";
									}
									else
										cout << "Error occured due to invalid input\n";
								}
